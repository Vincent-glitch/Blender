...
STAR_SCALE      = 500.0   # star density
STAR_SPREAD     = 0.003   # lower = more stars

FOCUS_TARGET    = PLANET_LOC
CAMERA_START    = PLANET_LOC + Vector((-25, -18, 6))
CAMERA_END      = PLANET_LOC + Vector((6, 4, 2))
...
def create_asteroid_field(count=ASTEROID_COUNT, inner=ASTEROID_INNER, outer=ASTEROID_OUTER, zrange=ASTEROID_Z, center=SUN_LOC):
    asteroids = []
    for i in range(count):
        ...
        z = center.z + random.uniform(-zrange, zrange)
...
create_asteroid_field(center=SUN_LOC)
